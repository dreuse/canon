...
function Collections() {
  ...
  // Document counts come from Collection.documents, which is otherwise only
  // loaded on expand — fetch each structure once so every row can show a count
  // at rest.
  const requestedCounts = useRef(new Set<string>());

  useEffect(() => {
    orderedCollections.forEach((collection) => {
      if (requestedCounts.current.has(collection.id)) {
        return;
      }
      requestedCounts.current.add(collection.id);
